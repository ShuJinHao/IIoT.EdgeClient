...
- 补传任务：`CloudRetryTask`
- 失败来源：Cloud consumer 上传失败、队列溢出持久化、fallback 恢复失败、反序列化失败、容量阻塞。
...
- 补传任务：`MesRetryTask`
- 失败来源：MES consumer 上传失败、队列溢出持久化、fallback 恢复失败、反序列化失败、容量阻塞。
...
1. `ProcessQueueTask` 或 `IngressOverflowPersistence` 发现 durable consumer 失败。
2. 根据 consumer 的 `DataPipelineRetryChannel` 选择 Cloud 或 MES。
3. 优先写对应链路的 retry 表，记录 `ProcessType`、`CellDataJson`、`FailedTarget`、`ErrorMessage` 和下次重试时间。
4. retry 表不可写时，写对应链路的 fallback 表。
5. 容量阻塞、fallback 也不可写、或 retry/fallback 中的 JSON 无法反序列化时，写对应链路的 deadletter 表。
6. deadletter 也失败时，才写 critical fallback 文件。

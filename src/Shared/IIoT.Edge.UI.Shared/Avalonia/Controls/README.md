...
- `Status`：`EdgeStatusDot`、`EdgeStatusChip`、`EdgeStatusPill`、`EdgeStatusListItem`、`EdgeVisualStatus`、`EdgeVisualVariant`
...
- 新增状态展示使用 `EdgeStatusDot`、`EdgeStatusChip`、`EdgeStatusPill`、`EdgeStatusListItem`。
...
- `EdgeStatusDot` 用于单点状态，`EdgeStatusChip` 用于短标签，`EdgeStatusPill` 用于胶囊状态，`EdgeStatusListItem` 用于状态摘要行；不得新增 `EdgeOnlineBadge`、`EdgeErrorTag` 这类场景状态控件。
...
`Avalonia/Styles/EdgeControls.axaml` 是当前唯一真实共享控件样式入口。`Styles/Controls/*.axaml` 拆分只有在某一类控件样式真实迁移完成并通过构建后才允许引入，禁止保留空壳 include。

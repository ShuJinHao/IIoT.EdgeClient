...
public sealed class RuntimeLayoutSyncExternalPluginBehaviorTests
{
    ...
    public void Run_WhenRuntimeLayoutIsRefreshed_ShouldPublishSingleHostAndPreserveDataDirectory()
    {
        ...
        try
        {
            ...
            WriteText(Path.Combine(hostRoot, "Modules", "TestPlugin", "plugin.json"), "{}");
            WriteText(dataSentinelPath, "runtime data content");
            WriteText(Path.Combine(repoRoot, "config", "appsettings.machine.TestPluginLine.json"), "{}");
            WriteText(
                  ...
                  "profiles": [
                    {
                      ...
                      "machineConfig": "config/appsettings.machine.TestPluginLine.json",
                      "moduleIds": [ "TestPlugin" ]
                    }
                  ]
                }
                """);
            ...
            Assert.True(File.Exists(Path.Combine(pluginsRoot, "TestPlugin", "plugin.json")));
    ...
    public void Run_WhenShellSourceIsHostDirectory_ShouldNotCleanHostOutput()
    {
        ...
        try
        {
            ...
            Assert.False(Directory.Exists(Path.Combine(hostRoot, "Modules")));
            Assert.True(File.Exists(Path.Combine(hostRoot, "appsettings.machine.TestPluginLine.json")));
            Assert.True(File.Exists(Path.Combine(pluginsRoot, "TestPlugin", "plugin.json")));
        }
        finally
        {

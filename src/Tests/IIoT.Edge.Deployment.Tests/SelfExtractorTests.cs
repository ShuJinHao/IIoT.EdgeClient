...
namespace IIoT.Edge.Deployment.Tests;
...
public sealed class SelfExtractorTests
{
    ...
    public void AppendThenReadAndExtract_ShouldRoundTripPayload()
    {
        ...
        try
        {
            ...
            using (var ms = new MemoryStream())
            {
                using (var zip = new ZipArchive(ms, ZipArchiveMode.Create, leaveOpen: true))
                {
                    WriteEntry(zip, "launcher/iiot-binding.json",
                        "{\"bindings\":[{\"moduleId\":\"TestPlugin\",\"clientCode\":\"DEV-AAAAAAAAAA\"}]}");
                    WriteEntry(zip, "host/IIoT.Edge.Shell.dll", "shell-bytes");
                    WriteEntry(zip, "plugins/TestPlugin/plugin.json", "{}");
                }
            ...
            var pluginPath = Path.Combine(installDir, "plugins", "TestPlugin", "plugin.json");
    ...
    [Fact]
    public void LegacyRawBindingCopy_ShouldBeProhibited()
    {
        var tempDir = CreateTempDir();
        ...
        try
        {
            ...
            var payloadPluginRoot = Path.Combine(payloadRoot, "plugins", "TestPlugin");
            ...
            Assert.Contains("raw iiot-binding.json", exception.Message, StringComparison.Ordinal);
            Assert.False(File.Exists(Path.Combine(launcherDataRoot, "iiot-binding.json")));
            Assert.False(File.Exists(EdgeClientProgramDataPaths.ResolveRuntimeBindingPath(currentRoot)));
        }
        finally
        {
            Environment.SetEnvironmentVariable(

...
public sealed class IoViewViewModelBehaviorTests
{
    ...
    {
        var model = new IoSignalModel { Remark = "测试插件 - 配方时间" };

        Assert.Equal("测试插件 - 配方时间", model.Remark);
        Assert.Equal("测试插件 - 配方时间", model.MatrixColumnTitle);
    }
    ...
    public async Task LoadDevicesAsync_WhenDevicesLoaded_ShouldShowConfiguredPlcs()
    {
        ...
        Assert.Equal(
            ["PLC-TestPlugin-01", "PLC-TestProcess-01", "PLC-TestProcess-02", "PLC-TestProcess-Disabled"],
    ...
    public async Task LoadMappingsAsync_WhenInteractionUsesSameBusinessGroup_ShouldMergeReadAndWriteIntoOneRow()
    {
        ...
        var mappings = new Dictionary<int, List<IoMappingEntity>>
        {
            [device.Id] =
            [
                CreateMapping(device.Id, "TestPlugin.Interaction.Inbound", "D701", 1, "Int16", "Read", "信号交互", "扫码进站", 10, "进站触发"),
                CreateMapping(device.Id, "TestPlugin.Interaction.InboundReply", "D601", 1, "Int16", "Write", "信号交互", "扫码进站", 11, "进站应答"),
    ...
    public async Task LoadMappingsAsync_WhenSameSignalConfiguredOnMultiplePlcs_ShouldUseSelectedPlcSavedAddress()
    {
        ...
        Assert.NotEqual("D701", Assert.Single(viewModel.InteractionRows).PlcAddressSummary);
    }
    ...
    public async Task LoadMappingsAsync_WhenContinuousSignalsShareGroup_ShouldBuildContinuousReadSection()
    {
        ...
        var mappings = new Dictionary<int, List<IoMappingEntity>>
        {
            [device.Id] =
            [
                CreateMapping(device.Id, "TestPlugin.Recipe.Time", "ZR0", 3, "UInt16", "Read", "连续读数据", "配方数组", 1, "配方时间"),
                CreateMapping(device.Id, "TestPlugin.Recipe.Temperature", "ZR100", 3, "Int16", "Read", "连续读数据", "配方数组", 2, "配方温度")
            ]
        };
        var dataStore = new PlcDataStore();
        ...
        Assert.DoesNotContain("TestPlugin.", string.Join(",", section.Signals.Select(static x => x.MatrixColumnTitle)));

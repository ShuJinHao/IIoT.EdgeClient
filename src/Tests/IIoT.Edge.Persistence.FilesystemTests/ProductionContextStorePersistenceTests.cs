...
public sealed class ProductionContextStorePersistenceTests
{
    ...
    public void GetOrCreate_WhenExistingBaseContextNeedsModuleContext_ShouldPreserveRuntimeState()
    {
        ...
        try
        {
            ...
            baseContext.SetStep("TestPlugin.Inbound", 30);
            ...
            Assert.Equal(30, typedContext.GetStep("TestPlugin.Inbound"));
    ...
    public void SaveToFile_WhenTempWriteFails_ShouldDeleteResidualTmpFileAndLogCleanupResult()
    {
        ...
        try
        {
            ...
            var exception = Assert.Throws<IOException>(store.SaveToFile);

            var tempPath = Path.Combine(tempDir, "production_context.json.tmp");
            Assert.False(File.Exists(tempPath));
            Assert.Contains("写入临时文件", exception.Message, StringComparison.Ordinal);
            Assert.Contains(
                logger.Entries,
    ...
    public void SaveToFile_WhenReplaceFails_ShouldDeleteTmpFileAndKeepExistingPersistedFile()
    {
        ...
        try
        {
            ...
            Assert.Contains("替换持久化文件", exception.Message, StringComparison.Ordinal);
            Assert.Contains(
                logger.Entries,
    ...
    {
        public const string TestModuleId = "TestPlugin";

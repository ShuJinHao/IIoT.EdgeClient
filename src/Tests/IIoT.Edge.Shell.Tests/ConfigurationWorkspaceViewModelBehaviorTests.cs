...
public sealed class ConfigurationWorkspaceViewModelBehaviorTests
{
    ...
    public void ConfigurationWorkspace_ShouldExposeRegisteredPluginConfigurationTabs()
    {
        ...
        Assert.Collection(
            viewModel.Tabs,
            tab => Assert.Equal("Homogenization.IOView", tab.ViewId),
            tab => Assert.Equal("Homogenization.RecipeView", tab.ViewId),
            tab => Assert.Equal("Homogenization.ParamView", tab.ViewId),
            tab => Assert.Equal("Homogenization.HardwareConfigView", tab.ViewId),
            tab => Assert.Equal("Homogenization.PlcTaskBindingView", tab.ViewId));
        Assert.Equal("Homogenization.IOView", viewModel.SelectedTab?.ViewId);
    ...
    public void ConfigurationWorkspace_ShouldUseMenuRequiredPermission()
    {
        ...
        var hardwareTab = viewModel.Tabs.Single(tab => tab.ViewId == "Homogenization.HardwareConfigView");
        ...
        var ioTab = viewModel.Tabs.Single(tab => tab.ViewId == "Homogenization.IOView");
    ...
    public void ConfigurationWorkspace_ShouldStopWhenMultiplePluginConfigurationGroupsExist()
    {
        ...
        registry.RegisterMenu(CreateMenu("Homogenization.IOView", "Navigation_Menu_Io", "IO", 3));
    ...
    {
        var registry = new FakeViewRegistry();
        registry.RegisterMenu(CreateMenu("Homogenization.ParamView", "Navigation_Menu_ParamConfig", "参数", 6, Permissions.ParamConfig));
        registry.RegisterMenu(CreateMenu("Homogenization.IOView", "Navigation_Menu_Io", "IO", 3));
        registry.RegisterMenu(CreateMenu("Homogenization.PlcTaskBindingView", "Navigation_Menu_PlcTaskBinding", "绑定", 8, Permissions.HardwareConfig));
        registry.RegisterMenu(CreateMenu("Homogenization.RecipeView", "Navigation_Menu_Recipe", "配方", 5));
        registry.RegisterMenu(CreateMenu("Homogenization.HardwareConfigView", "Navigation_Menu_HardwareConfig", "硬件", 7, Permissions.HardwareConfig));
        registry.RegisterMenu(CreateMenu("Homogenization.DataView", "Navigation_Menu_Data", "数据", 1));

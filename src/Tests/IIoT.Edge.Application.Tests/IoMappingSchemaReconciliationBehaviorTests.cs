...
public sealed class IoMappingSchemaReconciliationBehaviorTests
{
    ...
    {
        var device = CreatePlc(7, "PLC-TestModule");
        ...
        var profiles = new IModuleHardwareProfileProvider[] { new TestHardwareProfileProvider() };
        ...
        Assert.Contains(ioRepo.Items, x =>
            ...
            && x.SignalKey == "TestModule.Interaction.Inbound"
            && x.Direction == "Read"
            && x.PlcAddress == "D999");
    ...
    {
        var device = CreatePlc(8, "PLC-TestModule");
        var networkRepo = new InMemoryRepository<NetworkDeviceEntity>(device);

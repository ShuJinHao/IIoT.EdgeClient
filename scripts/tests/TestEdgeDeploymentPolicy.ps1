...
Require-Text '.github/workflows/edge-smoke-build.yml' 'TestEdgeDependencyClosureBehavior\.ps1' 'Affected deployment CI must execute the exact dependency-removal approval behavior tests.'
Require-Text 'scripts/TestEdgeDependencyClosure.ps1' 'does not exactly match disappeared libraries' 'Dependency closure removals must remain bound to an exact approved library set.'
Require-Text 'scripts/edge-dependency-removal-approvals.json' 'EDGE-DEPLOY-SECURITY-001' 'Intentional dependency removals must cite the governing security gate.'
Require-Text 'scripts/PublishEdgeRuntime.ps1' '-CandidateVersion \$Version' 'Runtime publishing must bind dependency-removal approval to the exact package version.'
Require-Text 'scripts/TestEdgeClientInstallerArtifact.ps1' '-CandidateVersion \$ExpectedVersion' 'Installer validation must bind dependency-removal approval to the exact package version.'
Require-Text 'scripts/TestEdgeVelopackPackage.ps1' '-CandidateVersion \$Version' 'Velopack validation must bind dependency-removal approval to the exact package version.'
Require-Text '.github/workflows/edge-pack-modules.yml' 'TestEdgeRuntimePublish\.ps1[^\r\n]*-Version \$env:EDGE_RELEASE_VERSION' 'Release CI runtime smoke validation must propagate the exact package version.'
Require-Text '.github/workflows/edge-pack-modules.yml' 'PublishEdgeRuntime\.ps1[^\r\n]*-Version \$env:EDGE_RELEASE_VERSION' 'Release CI runtime publishing must propagate the exact package version.'

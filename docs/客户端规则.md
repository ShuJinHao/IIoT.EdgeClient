...
- PLC 业务链路的隔离边界是单台 PLC：任务绑定是配置开关，某 PLC 未绑定某工序任务时不得启动该工序任务链；某 PLC 未连接、只读数据过期或主批计划门禁未满足时，不得为该 PLC 产生本地生产记录、Excel/本地业务归档、MES/Cloud 业务上传记录、补传记录或死信记录。允许 MES/Cloud 出口使用全局有界队列，但入队记录、补传记录、死信记录和幂等 key 必须携带 `NetworkDeviceId`、`DeviceName`、`ModuleId`、`TaskKey`、主批计划会话和追溯批次号，消费者不得用当前 UI 设备号或全局当前设备猜测记录来源。
...
- 生产业务运行任务不得直接调用 MES/Cloud 上传 HTTP、`IProcessMesUploader` 或 `IProcessCloudUploader`；必须先生成插件 `CellDataBase` 子类，设置 `UploadTargets`，通过工序门禁后再进入 `IDataPipelineService`。主批计划、PLC 连接、任务绑定或数据新鲜度门禁失败时不得入队，不能等 consumer 阶段再判断主批计划。

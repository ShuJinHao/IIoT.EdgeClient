...
- Required Edge tests 禁止 `Skip`、`Explicit`、`SkipWhen`、`SkipUnless`、`SkipExceptions`、运行期 `Assert.Skip` 及 data-row 等价禁用语义；只允许登记的 xUnit Fact/Theory 与 AvaloniaFact/AvaloniaTheory。测试树和每个构建输出只能存在唯一、哈希固定且 `failSkips=true` 的通用 `xunit.runner.json`，禁止程序集专用 `*.xunit.runner.json`、项目覆盖、VSTest `.runsettings`/`RunSettingsFilePath` 或 workflow `--settings` 第二配置源。metadata scanner 必须使用固定 SDK/`MetadataLoadContext` 二进制和 `CustomAttributeData`，真实序列化 constructor/named argument 与 InlineData payload，不得实例化测试 attribute、执行目标程序集或用反射对象 `ToString()` 代替业务参数；真实 runner discovery 是独立对账层，7 个项目的 runner count/digest 必须逐项等于 metadata projection/baseline。
...
- 根 `Directory.Build.targets` 必须在每个测试项目 `Compile` 后执行 assembly metadata gate，并在复制输出后核对唯一 runner 配置。仓库资产以 Git tracked + 非 ignore 未跟踪文件为真值，tracked 文件不得因放进 `bin`、`obj`、点目录、`artifacts` 或 `publish` 而逃过项目/源码/build graph 扫描；`src/Tests` 下的项目文件集合必须与受审 7 个 csproj 精确一致。非 C# 测试项目、显式/属性化 Import 或 PackageReference identity、未进入精确 allowlist/hash 的项目 MSBuild Target、嵌套 targets、关闭/重定向 root targets、测试项目移出 `src/Tests`、省略唯一直接 `IsTestProject=true`、项目级 runner override 都是旁路并必须失败。`.gitattributes`、`global.json`、`Directory.Packages.props`、`NuGet.Config`、根 props/targets、test csproj、runner 配置、测试源码正文、workflow 与 CODEOWNERS 均须精确受审。两份 Edge workflow 的 required Windows job 必须保持受审完整 job body、完整 step 集合/顺序、无 `if`/`continue-on-error`/额外 mutation step，并直接拥有 `timeout-minutes: 25`；执行顺序固定为 baseline anchor、治理自测/static、单次 Release solution build、跨项目重复与 discovery 对账，再按“legacy RepositoryHygiene architecture-policy lane → legacy Module.Contract gate → 其余业务/UI/回归”运行全部 7 个项目；前两项仍是待物理拆分的混合入口，不能称为目标 Architecture/Conformance 项目已经建成。远端 `main` 必须配置 required smoke check、禁止 force/direct push/admin bypass并要求非作者 Code Owner；只有作者一个 Owner 时不得开启后自锁，也不得把本地 CODEOWNERS 文件冒充已生效的 trust root。

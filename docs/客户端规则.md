...
- 历史复盘不是默认必读材料。修复历史回归、修改已冻结业务链路、当前实现与专题契约冲突、测试失败原因无法从源码和契约确定、同类问题曾经发生，或用户明确要求追溯历史决策时，必须按模块名、Rule ID、错误码、关键类型或故障症状定向检索 `改动复盘与规则沉淀.md`；不得用全文历史噪声替代当前正式规则和专题契约。
...
- 异步测试必须等待本用例承诺的完整语义状态，而不是只等待最早出现的中间副作用。生产路径会先记录请求/入队、随后更新结果、诊断或回调状态时，测试必须用可观察 completion、`TaskCompletionSource`、受控时钟或组合谓词等待全部断言前置状态；禁止用固定 `Task.Delay`、重复重跑或只观察第一条记录掩盖半完成竞态。该通用要求归入 `EDGE-TEST-002`，具体匀浆实时上传回归仍由 `EDGE-TEST-FLAKE-001` 跟踪。

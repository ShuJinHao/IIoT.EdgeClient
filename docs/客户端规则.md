...
- 新播种的 PLC 设备必须默认关闭；管理员完成现场地址、端口和 IO 核对并启用 PLC 后，已启用的默认任务绑定才允许随该 PLC runtime 启动。
...
- MG1、MG2 绑定继续按现有插件声明播种；设备状态第三任务只有在 PLC 方正式确认状态点位语义并完成 Cloud 专项契约后才能加入，当前禁止提前制造半条状态上传链。
...
- MG1 任务必须通过统一 PLC 请求协调器按需读取 MG1 弹夹码；MG2 任务必须独立按需读取 MG2 弹夹码，禁止周期任务无条件读取两个槽位的弹夹码。

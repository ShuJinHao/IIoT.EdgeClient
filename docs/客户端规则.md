...
- 首装包中 `launcher/iiot-binding.json` 是唯一允许携带 `BootstrapSecret` 的待应用输入；Launcher 只消费这一总绑定并按条目事务写入 profile 外部配置。Host/Plugin 选择清单不得携带密钥，客户端不得读取或恢复历史 `iiot-plugin-binding.json` 作为第二绑定源。

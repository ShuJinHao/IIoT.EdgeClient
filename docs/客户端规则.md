...
- `scripts/tests/edge-test-inventory.json` 是当前测试项目清单，必须精确登记 solution 项目数、全部 required 测试项目、分类、运行时和测试 fixture；当前真值为 solution 61 个项目、32 个 required runner、1 个仅测试插件 fixture，Release 发现 1332 case。`IIoT.Edge.Module.TestPlugin.Companion` 是 plugin-owned TestSupport，不是第二个 fixture 或 runner。Unit 只允许 `Pure + Parallel`，其他 Pure runner 也受控并行，Filesystem/SQLite/Avalonia/Windows 必须物理命名且 `Serial`。新增、删除或迁移项目时必须在同一批次更新并通过 `Get-EdgeTestInventory.ps1` 对账。

...
- Launcher 卡片来自 `launcher.profiles.json`，该文件由 `scripts/edge-runtime.publish.json` 生成；不得新增工序专属 Launcher XAML。
- 机器 profile 的 `Modules:Enabled` 决定当前 runtime 启用哪些模块。
...
- 发布 runtime 是目录树，不是单一共享根：`launcher/`、`homogenization/` 等 runtime 目录分离。
- 每个工序 runtime 只保留自己的模块；进程隔离由 runtime 目录、`Shell__MachineProfile`、`Shell:RuntimeDataRoot` 保证。
- 仓库只保留 `launcher.accounts.sample.json`；真实 `launcher.accounts.json` 不提交。
- 新增工序 runtime 时，更新模块项目、machine profile、`scripts/edge-runtime.publish.json`，再运行发布脚本。

...
- DataPipeline durable consumer 只有在调用方/runtime token 已请求取消时，`OperationCanceledException` 才表示正常停机；该异常必须原样退出当前消费，禁止写 retry、fallback、deadletter 或 critical fallback。取消退出还必须从目标内存 outlet queue 移除尚未开始的 item 并结清 pending 计数，且 dispatch 与 drain 必须同步，不能让取消后的晚投递永久悬挂；该清理不代表 shutdown durable，也不能把入队 acceptance 改写成已持久化。若 consumer/provider 自行抛出 `OperationCanceledException` 而调用方 token 未取消，则仍是普通失败，必须只进入该目标出口的一次补偿。取消测试必须使用独立 SUT token、确定性双 item/idle barrier，并在 `finally` 中取消且等待 runtime 收口；禁止用不可取消 token 直接调用单周期入口后遗留后台 worker。
...
- `scripts/tests/edge-test-inventory.json` 是当前测试项目清单，必须精确登记 solution 项目数、全部 required 测试项目、分类、运行时和测试 fixture；当前真值为 solution 32 个项目、8 个 required 测试项目和 1 个仅测试插件 fixture。新增、删除或迁移项目时必须在同一批次更新并通过 `Get-EdgeTestInventory.ps1` 对账。
...
- 测试分类用于表达失效机制和运行时路由，不得用 `Phase*`、`Batch*`、`General`、`Misc` 或 `NonUi` 作为新增分类；`Regression` 只保留给尚未完成物理归口的旧桶，新测试应进入 Unit、Architecture、Aggregate、Application、Contract、Conformance、Persistence、Workflow、Startup、UI、Deployment 或 Golden 的对应 owner。
...
- 两份 Windows CI job 继续保持 25 分钟 hard timeout、无 soft-fail，并运行 Release build、Analyzer/project-graph 与真实正反例 build fixture、inventory、真实 discovery、全部 8 个 required 测试项目和 TRX 对账；不得按 filter 把一个项目拆成多份后用人工加总冒充一次完整执行。
- `IIoT.Edge.Architecture.Analyzers` 必须作为 Analyzer 引用随所有生产项目构建执行，`IIoT.Edge.Architecture.AnalyzerTests` 必须保持 `Architecture` + `Pure/Roslyn`。当前启用的 17 个 compiler diagnostics 和项目图 `WSARCH001` 门禁以 `docs/Edge架构边界契约.md` 为唯一语义来源；未启用的 Rule ID 仍是后续批次，不得因本批已建 Analyzer 而冒充 Edge 全部架构治理完成。
...
pwsh -NoLogo -NoProfile -File IIoT.EdgeClient/scripts/tests/Update-EdgeRequiredTestCounts.ps1 -Configuration Release -OutputPath artifacts/required-test-counts.candidate.json

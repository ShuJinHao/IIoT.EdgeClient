...
- `EDGE-BASELINE-MIG-BOOTSTRAP` 完成前，新增 Analyzer/AnalyzerTests、物理拆分测试项目或其他 Phase 0 受保护资产变化一律禁止，不能用尚不存在的 receipt 消费能力放行。bootstrap 完成后，合法基线演进必须先由独立提交把精确迁移 receipt 送入可信 base，再由后续提交消费；禁止同一候选改 policy、改 baseline、改项目图并自我批准。Cloud 的历史 force-with-lease 授权不得自动扩大到 Edge。
...
- `EDGE-TEST-GOV-001` 是 Edge 测试资产止血 hard gate。受版本控制的 Phase 0 baseline 当前固定 7 个测试项目、964 个声明、1010 个 concrete execution template 和 1091 个 runner case；baseline 只能由人工显式命令生成候选，CI 禁止自动重写。PR/main push 必须从 GitHub event 的 base/before commit读取可信旧 baseline 并要求候选完全相同；tag/workflow_dispatch 必须 fetch `origin/main`，手工入口只允许 `refs/heads/main`，tag commit 必须是受保护 main 可到达的祖先。同一变更内重生成 baseline 或同步修改 policy hash 不构成批准；Phase 0 基线演进必须等待独立的 migration receipt/ledger 批次。baseline、waiver、policy/behavior、根 `Directory.Build.targets`、solution 和两份 workflow 必须进入 Code Owner 审核；普通测试运行不得修改任何基线。

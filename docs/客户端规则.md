...
- 首装包中 `launcher/iiot-binding.json` 是唯一允许携带 `BootstrapSecret` 的待应用输入；Launcher 只消费完整合法的 `schemaVersion=2`，必须先同时校验 HTTP(S) `baseUrl`、`generatedAtUtc`、全部绑定身份以及 `deviceInstance/clientReleaseCatalogTemplate/clientVersionReport/runtimeHeartbeat` 四条相对路径，其中 catalog 模板必须且只能含一个 `{deviceId}`。校验通过后，身份和四条 Cloud-owned 路径必须在每个 profile 外部 machine config 内一次原子写入，覆盖四条旧路径但保留其它现场配置。Host/Plugin 选择清单不得携带密钥，客户端不得读取或恢复历史 `iiot-plugin-binding.json` 作为第二绑定源。
...
- Cloud 配置写入失败必须进入明确诊断并保留可恢复输入，禁止静默应用半份绑定。schema 不支持或任一必需字段不合法时，不得启用 Cloud、消费 pending 或生成成功摘要；不得恢复 Host `appsettings` fallback 或要求现场手填。

#### HL-META-001：插件静态元数据不得成为第二配置源

- `plugin.json` 只能声明模块身份、显示信息、入口、版本、Host 兼容范围、正式配置 schema 路径、版本化 `ModuleSeed` 策略和静态能力；不得直接承载 PLC、IO、MES 或 Business 运行值。
- activation machine profile 只能声明 profile、固定 Host 入口、独立 `InstanceId`、所属模块以及 `ModuleSeed` 版本和环境选择，不得重复保存设备、IO 或本地参数默认值。
- 插件可以携带版本化的 PLC/IO、MES 和 Business 本地播种定义，但必须通过客户端统一播种端口写入正式存储。
- 旧 `*.module.json` 不再是运行配置源；即使历史包仍携带该文件，Host 也只能报告稳定诊断并忽略其全部键。
- `plugin.json`、activation、Cloud 绑定文件和本地数据库之间必须有明确且唯一的 owner，禁止同一参数在多处分别生效或按偶然加载顺序覆盖。
...
- 最终启动校验必须在取得共享 launch/update 租约后、启动进程前重新读取选择清单和当前 activation，并持有租约完成 Ready 交接。缓存卡片必须携带贡献它的 `ModuleId + pluginDirectory`；只有当前选中目录仍贡献同一 `ProfileId/MachineProfile` 才能启动，禁止在租约外预检后使用可能已被更新替换的 profile、目录或模块身份。
...
- release bootstrap、catalog、包下载和版本上报属于发布控制面；当前 profile 存在合法首装绑定和正式外部更新配置时必须继续工作。`FileEdgeUpdateConfigurationProvider` 必须同时取得设备 bootstrap、设备 catalog、版本上报和运行心跳四条正式路径；缺任一项时整份发布控制配置不可用，不得从 Host 默认值补齐。
...
- 插件负责声明本工序的正式参数 schema、版本化 `ModuleSeed`、默认值、PLC/IO 模板和初始化数据；Host 校验 schema/manifest 身份及安全策略，并只提供通用注册、事务和持久化端口。
- 同一模块只能有一个正式配置 Owner；schema 缺失、身份不一致、重复 Owner 或 machine profile 选择了不受支持的播种版本/环境时必须形成模块级诊断，禁止回退到旧默认文件。
...
- 每次播种必须绑定 manifest 声明且 machine profile 明确选择的 `ModuleSeed` 版本与环境；未选择或不匹配时只隔离该模块并允许 Shell 进入诊断态。
...
- Host 内部初始化器、插件 contributor、代码命名和日志必须使用 `ModuleSeed` 或“插件配置初始化/播种”等生产语义；Host API 2.0.x 为二进制兼容保留的旧 SDK 注册槽不得重新成为样例语义或运行配置旁路。
...
- Cloud 登录和 refresh 返回的 access token 在 Edge 中是不透明凭据；建立客户端授权会话前，必须携带完全相同的原始 bearer token 调用 Cloud trusted session 接口，由 Cloud JwtBearer 完成签名、issuer、audience 和 lifetime 验证并返回当前服务端身份事实。Cloud 拒绝、网络失败或服务端投影无效时必须返回令牌无效并保持无会话，不得阻断 Launcher/Shell 启动。
- Cloud 当前使用服务端对称签名密钥，禁止把 JWT signing key 下发或写入 Edge 配置、环境变量、machine profile、安装包和现场文件。生产 Edge 不得读取、解析或信任 access token 内的 JWT claims/`exp`；Cloud 登录和刷新后必须从 `X-IIoT-Access-Token-Expires-At` 取得过期时间，并使用原始 bearer token 请求 `GET /api/v1/human/identity/session`，只以该服务端投影返回的实时账号、员工、角色和权限事实构造 `UserSession`。缺失或过期的 expiry header、Cloud 拒绝、状态服务不可用、session DTO 缺字段或畸形都必须失败关闭；未来只有 Cloud 正式提供非对称公钥验证契约时才允许重新设计本地验签。
...
- 任一必需本地 consumer 失败时必须保留入口信封和已完成 consumer 回执；恢复时只重试未完成项。外部目标只有真实接收成功，或已可靠落入本通道 retry/fallback/deadletter 可消费链，才能记为交接完成。
...
- AP/CP 过站 Cloud 适配器必须按记录逐条执行 v2 严格校验：`processType`、稳定 `plcCode`、真实 `plcName`、MG1/MG2、OK/NG、UTC 时间顺序和 schema 字段均满足时才进入 `schemaVersion=2` 批次；历史不完整记录继续按 v1 独立分批，禁止为了升级版本补造字段或混入 v2 批次。
...
- AP/CP Cloud uploader 还必须在同一 `ProcessType + PlcCode` 边界内按过站 v1/v2 资格再次拆批；v2 资格校验失败只能降为兼容 v1，不得改变原始业务事实。各批使用稳定且互不碰撞的请求身份，ACK 丢失或重试仍由 Cloud 内容指纹与单条业务哈希保证不重复落库。

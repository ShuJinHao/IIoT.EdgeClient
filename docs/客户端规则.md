...
- 旧产能缓冲中的历史 PLC 身份只允许先精确匹配当前 `PlcCode`，再匹配持久化别名表中唯一且未被其它 `PlcCode` 复用的已验证别名；当前 `DeviceName` 不得直接认领旧记录。别名文件损坏、权威 PLC 配置读取失败、别名复用或无法唯一解析时，必须保留原产能记录、不上传、不删除，并释放本轮领取状态等待人工修复。

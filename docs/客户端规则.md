...
- 所有 `IPlcService` 协议实现都必须把 `ConnectAsync`、`Disconnect`、`ReadDataAsync`、`WriteDataAsync` 和 `Dispose` 纳入同一个 PLC 操作门控；读写必须进入门控后再获取当前协议连接或 master，禁止先取连接对象再等锁，避免读写过程中被并发断开、重连或释放。

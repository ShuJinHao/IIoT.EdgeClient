...
- 宿主、SDK 与私有插件三仓拆分由 `docs/Edge宿主SDK私有插件三仓拆分计划.md` 专门裁决。2026-07-22 已形成三个独立本地 Git 仓的源码候选：依赖方向唯一为 `Host -> SDK 包`、`Plugin -> SDK 包`，Host 与 Plugin 互不引用；当前统一状态是 `EDGE_LOCAL_THREE_REPOSITORY_CANDIDATE / NOT-VERIFIED`。旧 Phase 0–5 机器账本与 formal 规则只绑定冻结 worktree，不得在本候选中重生成或冒充新鲜验证；Phase 9 发布入口、Phase 10 Launcher 组合升级保护、远端建仓/remote/NuGet、push/PR/merge、`stable`、发布和部署均仍需单独授权。
...
- `scripts/tests/edge-test-inventory.json` 是 Host 仓当前测试项目清单，必须精确登记 Host solution 项目数、全部 Host required 测试项目、分类、运行时和测试 fixture。拆仓前最近可信完整基线为单 solution 61 个项目、32 个 required runner、1 个仅测试插件 fixture，Release 发现 1334 case（PR #53 基线 1332，当时新增 2 个 Phase 0 契约账本 Architecture 门禁）。2026-07-22 物理三仓源码候选的静态项目归属为 Host 50 个 solution project / 26 个 required runner、SDK 6 / 2、Private Plugins 6 / 4；三仓合计仍为 32 个 runner，但尚未重新执行 discovery/TRX，也未更新 `required-test-counts.json`、discovered inventory 或 coverage 等真实运行基线，因此这些数字只能标记 `NOT-VERIFIED`，不得称为本批 PASS。物理计划禁止为 SDK/插件另建平行测试治理清单，两个新仓暂以各自 solution 与测试项目 metadata 表达 owner，待获批 V1–V5 时统一做 before/after 对账。`IIoT.Edge.Module.TestPlugin.Companion` 是 plugin-owned TestSupport，不是第二个 fixture 或 runner。Unit 只允许 `Pure + Parallel`，其他 Pure runner 也受控并行，Filesystem/SQLite/Avalonia/Windows 必须物理命名且 `Serial`。新增、删除或迁移项目时必须在同一批次更新对应 owner 的项目清单；需要执行 `Get-EdgeTestInventory.ps1`、discovery 或 TRX 的对账必须服从当前批次验证授权。

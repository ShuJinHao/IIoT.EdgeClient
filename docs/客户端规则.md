...
- `IIoT.Edge.Architecture.Analyzers` 必须作为 Analyzer 引用随所有生产项目构建执行，`IIoT.Edge.Architecture.AnalyzerTests` 必须保持 `Architecture` + `Pure/Roslyn`。Analyzer 项目自排除必须以 `System.IO.Path.GetFullPath` 产生的规范化绝对项目路径比较，禁止用未经规范化的拼接路径或项目名排除；build fixture 必须锁定唯一表达式，并证明 Analyzer 项目在 Windows/Unix 路径语义下都不会形成 self `ProjectReference`。当前启用的 23 个 compiler diagnostics、160 条 AnalyzerTests、7 个 valid / 43 个 invalid 隔离 build fixture 和项目图 `WSARCH001` 门禁以 `docs/Edge架构边界契约.md` 为唯一语义来源；未启用的 Rule ID 仍是后续批次，不得因本批已建 Analyzer 而冒充 Edge 全部架构治理完成。

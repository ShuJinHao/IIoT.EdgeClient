...
- 恢复一致性与终止审计中的“现场读值”固定指同一原子成功新鲜快照内的弹夹码、数量、速度和采集 UTC。弹夹码、数量、速度任一变化，或旧检查点缺少数量、速度、采集时间中的任一项时，必须用该份完整观测更新检查点并递增 revision；三项完全相同时不得重复落盘或产生 revision 抖动。
...
- 任务绑定页面必须显示 SDK 公共恢复快照提供的检查点弹夹、现场弹夹码、恢复状态、检查点/读值时间和稳定原因码，并只在需要人工确认时显示操作入口。数量、速度保留在插件私有检查点和终止审计中，不扩展 SDK 公共恢复快照或单独投影到页面；但任一私有现场值变化都必须推进页面携带的 revision。确认命令必须携带页面所见 revision；页面打开后完整现场读值或恢复状态变化时，旧 revision 必须冲突、刷新并要求重新核对，禁止确认旧快照。

...
- 日常客户端快发允许操作者在本机运行 `scripts/LocalPublishAndDeploy.ps1`，本机完成编译、Velopack 打包和 installer artifact 生成后，通过 rsync/scp 发布到 `/srv/iiot/edge-updates`。这是本机运维发布路径，不属于 GitHub CI/CD job；Cloud catalog 会扫描落盘的 `installer-artifact.json` 并合并数据库 release 状态。

...
- `scripts/tests/edge-test-inventory.json` 是当前测试项目清单，必须精确登记 solution 项目数、全部 required 测试项目、分类、运行时和测试 fixture；当前真值为 solution 30 个项目、7 个 required 测试项目和 1 个仅测试插件 fixture。新增、删除或迁移项目时必须在同一批次更新并通过 `Get-EdgeTestInventory.ps1` 对账。
...
- 两份 Windows CI job 继续保持 25 分钟 hard timeout、无 soft-fail，并运行 Release build、inventory、真实 discovery、全部 7 个 required 测试项目和 TRX 对账；不得按 filter 把一个项目拆成多份后用人工加总冒充一次完整执行。
- 当前 inventory/count 门禁只负责项目集合、真实发现数和 runner 结果，不冒充完整的 DDD/分层/插件/DB Roslyn 语义门禁。`EDGE-ARCH-001` 仍须另建 Analyzer/project-graph hard gate，覆盖跨层依赖、循环依赖、绕过聚合根、直接 DB、插件反向引用、禁止 API/namespace 和生产引用 TestKit；未落地前不得宣称 Edge 架构治理完成。

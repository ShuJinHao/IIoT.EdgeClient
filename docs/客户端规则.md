...
- 日常客户端快发允许操作者在本机运行 `scripts/LocalPublishAndDeploy.ps1 -Transport http`，本机完成编译、Velopack 打包和 installer artifact 生成后，通过 Cloud Human API 上传 release bundle 到 `/srv/iiot/edge-updates`。这是本机运维发布路径，不属于 GitHub CI/CD job；未传 `-Version` 时脚本通过 Cloud Human catalog 读取 stable 最新版本并自动递增 patch；上传默认限速 100 Mbps、单并发、服务端审计，完成后必须打印部署总结和 HTTP 验证结果。`rsync/scp` 只作为 HTTP 发布不可用时的 fallback。Cloud catalog 会扫描落盘的 `installer-artifact.json` 并合并数据库 release 状态，文件存在性以落盘 bundle 为准，可见性和生命周期以 DB 状态为准。

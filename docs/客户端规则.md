...
- `scripts/tests/edge-test-inventory.json` 是当前测试项目清单，必须精确登记 solution 项目数、全部 required 测试项目、分类、运行时和测试 fixture；当前真值为 solution 61 个项目、32 个 required runner、1 个仅测试插件 fixture，Release 发现 1232 case。`IIoT.Edge.Module.TestPlugin.Companion` 是 plugin-owned TestSupport，不是第二个 fixture 或 runner。Unit 只允许 `Pure + Parallel`，其他 Pure runner 也受控并行，Filesystem/SQLite/Avalonia/Windows 必须物理命名且 `Serial`。新增、删除或迁移项目时必须在同一批次更新并通过 `Get-EdgeTestInventory.ps1` 对账。
...
- Analyzer 对生产 Task 到 outbound/DataPipeline sink 的判定必须跟踪 helper、interface 和 override 调用图；只有可达、不 rethrow 的通用异常处理才能切断违规路径，`catch` 后重抛或恒 false filter 不算处理。Plugin role、注册类和 sink 必须按精确 symbol/metadata/namespace 匹配，不得用类名后缀或简名误报。
- compatibility inventory 除 token 级 disposition 外，必须对每个真实兼容候选声明精确登记 symbol、declaration path、理由与调用证据；宽泛 fallback/wrapper disposition 不能掩护未登记或零调用 symbol，该类声明必须物理删除。
...
- `IIoT.Edge.Architecture.Analyzers` 必须作为 Analyzer 引用随所有生产项目构建执行，`IIoT.Edge.Architecture.AnalyzerTests` 必须保持 `Architecture` + `Pure/Roslyn`。当前启用的 23 个 compiler diagnostics、133 条 AnalyzerTests 和项目图 `WSARCH001` 门禁以 `docs/Edge架构边界契约.md` 为唯一语义来源；未启用的 Rule ID 仍是后续批次，不得因本批已建 Analyzer 而冒充 Edge 全部架构治理完成。

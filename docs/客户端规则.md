...
- coverage、duplication、mutation 与 compatibility 基线不得由同一变更自行放宽后再用当前值证明自己通过。required CI 必须把四份当前基线与 PR base 的已提交版本比较，BaseRef 必须是候选 HEAD 的真实祖先且不得等于候选 HEAD；若 base 尚无该文件，只允许使用本分支最早的已提交 bootstrap 版本作为不可变锚点，工作树内临时 bootstrap 直接失败。覆盖率/高风险阈值和 required report 数不得降低，重复检测窗口/最小文件数不得放宽且 clone group/instance 不得增长，mutation score 不得降低且 survived/no-coverage/timeout/compile-error 等失败上限不得增长，compatibility scan token 不得删除、候选/符号/迁移窗口/consumer 上限不得新增或增长、删除期限不得后移。真实删除和更严格证据允许收紧；禁止恢复 reviewer、CODEOWNERS 或其他人工审批链代替机械 before/after ratchet。

...
- 任务绑定页面必须显示 SDK 公共恢复快照提供的检查点弹夹、现场弹夹码、恢复状态、检查点/读值时间和稳定原因码，并只在需要人工确认时显示操作入口。数量、速度保留在插件私有检查点和终止审计中，不扩展 SDK 公共恢复快照或单独投影到页面；但弹夹码、数量或速度任一变化都必须推进页面携带的 revision。确认命令必须携带页面所见 revision；页面打开后三项业务读值或恢复状态变化时，旧 revision 必须冲突、刷新并要求重新核对，禁止确认旧快照；仅采集 UTC 推进而三项未变时，确认仍针对原已落盘观测。

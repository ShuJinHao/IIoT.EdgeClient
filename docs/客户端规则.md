...
- `src/Modules` 下必须显式区分插件入口工程和共享库工程：插件入口工程必须声明 `PluginModuleId`、`IsEdgePluginModule=true`、`IsPackable=true` 并携带 `plugin.json`；非插件共享库工程必须命名为 `*.Shared`、`IsEdgePluginModule=false`、`IsPackable=false` 且不得携带 `plugin.json`。共享库不得出现在 Launcher 卡片、插件 catalog 或发布插件包中。

...
- Analyzer、ArchitectureTests、ConformanceTests 和 evaluated project graph 只锁定架构契约，不冻结业务测试源码或 case roster。Unit/Aggregate/Application/Workflow/Contract/Persistence/Integration/UI 测试必须允许随功能同批增删改移；inventory/count/quality baseline 不是授权门，不得为业务测试恢复 transition registry、waiver、receipt 或其他平行审批链。
- 验证按“受影响项目/定向测试 → 所属 runner → 候选稳定后一次完整 required”执行；除真实跨仓契约变更外，不因 Edge 单仓业务改动反复跑三仓全量测试。
...
- 已物理退役功能不能只留一条人工说明。`EDGE-DIECUT-RETIRE-001` 继续由专用 retirement evidence 绑定既有 source commit/tree、精确 41 条旧声明、固定 4 个历史 `oldSourcePath`、受控 token/path pattern、逐条唯一 `oldKey`、精确 `retired-diecut` decision 和非空 reason，并证明旧声明没有回流当前 discovery。非文档 active input 的模切命中只允许专用证据文件 `scripts/tests/baselines/edge-retired-feature-evidence.json`；任何 `src`/测试/UI/solution/project/config/`.github`/打包发布输入或其它治理文件命中都必须由 `Test-EdgeRetiredFeatureEvidence.ps1` fail-closed。通用 regression ledger 已物理删除，不再把其它旧业务测试绑定到当前方法名。
...
- `IIoT.Edge.Module.Analyzers` 必须由 `IIoT.Edge.Sdk` 唯一拥有源码和 `Architecture + Pure/Roslyn` 测试；Host 只能从受控本地 package feed 以 `PrivateAssets=all` 的唯一 `PackageReference` 随所有生产项目构建执行，禁止恢复 Analyzer 源码副本、跨仓 `ProjectReference`、hint path 或可关闭开关。Host 的 compiler ID catalog 必须从当前中央版本对应 nupkg 内的 release catalog 读取，project graph 和隔离 build fixture 继续机械证明 Analyzer 未被禁用或降级。当前启用的 23 个 compiler diagnostics 与项目图 `WSARCH001` 门禁以 `docs/Edge架构边界契约.md` 为唯一语义来源；未启用的 Rule ID 仍是后续批次，不得因本批已建 Analyzer 而冒充 Edge 全部架构治理完成。

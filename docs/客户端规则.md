...
- 宿主、SDK 与私有插件三仓拆分由 `docs/Edge宿主SDK私有插件三仓拆分计划.md` 专门裁决。当前只授权在既有 `IIoT.EdgeClient` 仓执行 Phase 0–5；Phase 6–10、远端建仓、remote 变更、远端 NuGet 发布、生产部署、`stable` 与合并仍需后续当前轮明确授权。Phase 0–5 的唯一机器账本是 `eng/Generate-EdgePluginContractLedger.ps1` 与 `eng/baselines/edge-plugin-contract-ledger.json`；不得另写 namespace grep、临时扫描器或手工 JSON 替代 evaluated ProjectReference + Roslyn + PE AssemblyRef + package assembly 四层门禁。四层 owner family、输入明细和计数必须由 strict schema 与 validator 双重重算，package 在 Phase 0–3 明确为未适用，carry set 只能 exact 保留或按目标批次归零。每个 Phase 先形成 clean implementation commit，再生成 ledger 并以恰好一个 ledger-only evidence commit 收口；账本记录 implementation HEAD/tree，final canonical gate 必须证明唯一 parent、唯一 ledger 差异与最终 clean worktree，不得伪造包含自身的最终 commit hash，noncanonical/pending smoke 不算 final 证据。当前项目、源码、runner/case、manifest 或资源数不得写成后续 Phase 的永久常量。
- Phase 0–5 的正式 Git 收口顺序固定为：本批实现、测试、schema 与全部 tracked 文档进入 clean implementation commit `I`；唯一 generator 在 `I` 上只排除 canonical ledger 路径生成账本；唯一直接子提交 `E` 只含 mode `100644` 的该账本；最后在 `E` 的完全 clean worktree 上无参数执行 actual formal parent。`E` 之后不得补改 tracked 文档或代码；任何 tracked 变化都必须重新形成 `I → E → actual formal` 证据链。
- actual formal parent 必须是无参数、无 override、无 generator 的固定入口，在启动 authority 前执行两次相等的 clean `I/E` precondition，并在 authority 后验证签名收据、post-state 与精确清理。收据只是绑定本次 direct-child authority/replay 的临时机器事实，不是 reviewer、waiver、permission、人工批准或并行 authority 控制面；`cleanupComplete=true` 只表示本次受控临时运行根完成精确清理且仓库状态复核稳定，不表示删除最终收据。synthetic fixture、静态 mutation 或 schema 正负例都不能替代 actual formal 执行。
- schema v2 的 semantic evidence 必须保留源路径/位置、精确 symbol、symbol kind、完整 owner assembly identity 和 usage kind；carry item 以 `sourcePath + ownerAssembly + symbol` 为 canonical 聚合身份，occurrence 为其底层语义证据记录数，不得回退到只统计行、标识符或 invocation 的 stale schema v1 口径。本批审查接受的 final-canonical 候选为 `EDGE-SPLIT-020` 22 exact items / 122 semantic-evidence occurrences 和 `EDGE-SPLIT-030` 6 个私有 Presentation exact items / 11 semantic-evidence occurrences；只有 implementation/evidence pair 与 final validator 通过后才成为正式冻结值。
- `IIoT.Edge.UI.Shared` 是已批准的稳定 SDK UI surface。候选 schema v2 账本中 34 条 UiShared usages 必须作为 approved/stable contract 对账，不得划入 `EDGE-SPLIT-030` 待移除 carry；Phase 3 只对私有 Presentation seam 做迁移与归零，对 UiShared 做打包、API 和 runtime 边界化，不得重新标为 forbidden。
- `EDGE-SPLIT-020` 做 Hardware/development-sample DTO 迁移前必须先独立提交旧实现 canonical snapshot，expected 不得由新 mapper 同源生成。当前行为以源码和真实 SQLite 而非误导注释/内存仓为准：Reset 在读取 hardware profile 前删除当前库全部设备，并由 EF cascade 删除 IoMappings、PlcTaskBindings、DeviceParams；正常路径同名设备复用，任一既有映射使整套模板跳过，新设备与 mappings 当前是两个 UoW/两次 durable commit。迁移不得顺手缩窄 Reset；目标写入必须在单一 transaction/session 中完成，允许为 delete、identity、mapping 零到多次 non-durable flush，但最多一次 final durable commit，相同 PlcCode 重建和任一故障必须由 SQLite 证明整体 rollback 且零半成品。
...
- 工作区顶层入口向 Edge 内部执行器传递 `Deploy`、`ProfilePath`、release notes、Cloud token 等公共命名参数时，必须使用 PowerShell 命名 hashtable splatting；禁止用字符串数组位置 splatting 冒充命名参数。宿主 bundle 已包含并验证同一插件版本时，独立插件步骤必须记录 no-op reason `included-and-verified-in-edge-host-bundle`，不得重复构建或重复发布；纯插件变更仍只能走独立插件入口。
...
- `scripts/tests/edge-test-inventory.json` 是当前测试项目清单，必须精确登记 solution 项目数、全部 required 测试项目、分类、运行时和测试 fixture；历史冻结 inventory / 最近可信完整基线为 solution 61 个项目、32 个 required runner、1 个仅测试插件 fixture，Release 发现 1334 case（PR #53 基线 1332，当时新增 2 个 Phase 0 契约账本 Architecture 门禁）。当前 Gap A formal-close 候选未新增 Fact，但已修改既有 Architecture Fact，尚未重新执行 discovery/TRX；1334 只能作为当前 expected inventory，禁止称为本批 PASS。`IIoT.Edge.Module.TestPlugin.Companion` 是 plugin-owned TestSupport，不是第二个 fixture 或 runner。Unit 只允许 `Pure + Parallel`，其他 Pure runner 也受控并行，Filesystem/SQLite/Avalonia/Windows 必须物理命名且 `Serial`。新增、删除或迁移项目时必须在同一批次更新并通过 `Get-EdgeTestInventory.ps1` 对账。
...
- 已物理退役功能不能只在 regression ledger 写一条人工 decision。`EDGE-DIECUT-RETIRE-001` 必须由 schema v3 retirement evidence 绑定既有冻结 source commit/tree、精确 41 条旧声明、固定 4 个历史 `oldSourcePath`、受控 token/path pattern、逐条唯一 `oldKey`、精确 `retired-diecut` disposition/decision 和非空 reason，并证明旧声明没有回流当前 discovery。非文档 active input 的模切命中精确只允许 `scripts/tests/Test-EdgeRegressionLedger.ps1` 与 `scripts/tests/baselines/edge-regression-ledger.json`；任何 `src`/测试/UI/solution/project/config/`.github`/打包发布输入或第三个治理脚本命中都必须由 `Test-EdgeRetiredFeatureEvidence.ps1` fail-closed。正例和活动源码、第三治理文件、40/42、重复 key、错误 disposition/decision、旧声明回流等负例必须由独立 fixture 执行，且 smoke 与 pack workflow 只能在 preflight 验证，不能借此触发 pack/release/deploy。

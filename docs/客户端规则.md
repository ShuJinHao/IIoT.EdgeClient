...
- `scripts/tests/edge-test-inventory.json` 是当前测试项目清单，必须精确登记 solution 项目数、全部 required 测试项目、分类、运行时和测试 fixture；当前真值为 solution 61 个项目、32 个 required runner、1 个仅测试插件 fixture，Release 发现 1267 case。`IIoT.Edge.Module.TestPlugin.Companion` 是 plugin-owned TestSupport，不是第二个 fixture 或 runner。Unit 只允许 `Pure + Parallel`，其他 Pure runner 也受控并行，Filesystem/SQLite/Avalonia/Windows 必须物理命名且 `Serial`。新增、删除或迁移项目时必须在同一批次更新并通过 `Get-EdgeTestInventory.ps1` 对账。
...
- `IIoT.Edge.Architecture.Analyzers` 必须作为 Analyzer 引用随所有生产项目构建执行，`IIoT.Edge.Architecture.AnalyzerTests` 必须保持 `Architecture` + `Pure/Roslyn`。当前启用的 23 个 compiler diagnostics、157 条 AnalyzerTests 和项目图 `WSARCH001` 门禁以 `docs/Edge架构边界契约.md` 为唯一语义来源；未启用的 Rule ID 仍是后续批次，不得因本批已建 Analyzer 而冒充 Edge 全部架构治理完成。

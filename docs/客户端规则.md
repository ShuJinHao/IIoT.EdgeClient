...
- Host 仓不再拥有 `src/Modules` 下的 SDK 或具体插件项目。公共面由独立 `IIoT.Edge.Sdk` 仓的 `IIoT.Edge.Module.Contracts`、`IIoT.Edge.Module.Sdk`、`IIoT.Edge.UI.Shared`、`IIoT.Edge.Module.Analyzers` 四个 `2.0.0` 包提供；Host 与具体插件均只经 `PackageReference` 消费，禁止跨仓 `ProjectReference`、源码链接、DLL hint path、共享 `bin/obj` 或源码 fallback。插件仍须声明 `PluginModuleId`、`IsEdgePluginModule=true`、`IsPackable=true` 并携带 `plugin.json`，禁止用插件族 `*.Shared` 承载具体工序业务。
...
- 宿主、SDK 与私有插件三仓拆分由 `docs/Edge宿主SDK私有插件三仓拆分计划.md` 专门裁决。2026-07-22 已形成三个独立本地 Git 仓的源码候选：依赖方向唯一为 `Host -> SDK 包`、`Plugin -> SDK 包`，Host 与 Plugin 互不引用；本地 V1–V5 已完成独立 restore/build/pack、零源码 fallback 组合、各仓 owner runner、Host TRX/coverage 对账，当前状态为 `EDGE_LOCAL_THREE_REPOSITORY_CANDIDATE / V1-V5-LOCAL-PASS`。旧 Phase 0–5 机器账本与 formal 规则只绑定冻结 worktree，不得在本候选中重生成或冒充新鲜验证；Phase 9 发布入口、Phase 10 Launcher 组合升级保护、Windows 远端/实机、远端建仓/remote/NuGet、push/PR/merge、`stable`、发布和部署均未执行且仍需单独授权。
...
- `EDGE-SPLIT-020` 的 Hardware/development-sample 边界固定为“插件提交 DTO，Host 唯一端口物化 Domain”。Reset 仍删除当前库全部设备并由 EF cascade 处理关联；同名设备复用，任一既有映射使整套模板跳过。候选写入端口必须在单一 transaction/session 内完成，允许 delete/identity 的 non-durable flush，最多一次 final durable commit；相同 PlcCode 重建、rollback 与零半成品仍为 `NOT-VERIFIED`，只能由后续获批的真实 SQLite 验证确认。
...
- 插件包和 runtime plugins 目录必须由私有插件仓唯一 `eng/PackEdgePlugin.ps1` 按静态 allowlist 生成，不得复制整个 module build output；SDK、Application、Domain、Presentation、Host、Tests、开发配置和现场运行数据不得随插件重复分发。Host 只保留运行时 manifest/load/dependency/capability/ViewId 校验；发布入口改造属于 Phase 9，本候选不执行也不声称已接通。
- Host `RuntimeLayoutSync` 不得扫描、构建或复制 `src/Modules` 等插件源码输出。未进入 Phase 9/10 组合流程前，Host 的 `Default` profile 必须允许空 `moduleIds` 并只生成空插件根；任何非空模块组合必须在改写现有布局前 fail-closed，等待获批的正式插件 artifact 组合入口，禁止回退单仓源码构建。
...
- `scripts/tests/edge-test-inventory.json` 是 Host 仓当前测试项目清单，必须精确登记 Host solution 项目数、全部 Host required 测试项目、分类、运行时和测试 fixture。拆仓前冻结基线为单 solution 61 个项目、32 个 required runner、1 个仅测试插件 fixture，Release 发现 1334 case（其中 2 个 Phase 0 canonical-ledger Fact 只属于冻结证据拓扑）。2026-07-22 V5 已按物理 owner 证明 Host 为 50 个 solution project / 26 个 required runner / 1 个 fixture，SDK 为 6 / 2，Private Plugins 为 6 / 4；Host 当前 discovery、required counts、26 份 TRX 与 coverage reports 已机械对齐，SDK 2 个与插件 4 个 owner runner也已分别执行且 0 failed / 0 skipped。三仓合计仍是 32 个 owner runner，但不得把 Host 26 个 runner 冒充三仓合计，也不得把拆仓前 1334 case复制成当前真值。物理计划禁止为 SDK/插件另建平行测试治理清单，两个新仓以各自 solution 与测试项目 metadata 表达 owner。`IIoT.Edge.Module.TestPlugin.Companion` 是 plugin-owned TestSupport，不是第二个 fixture 或 runner。Unit 只允许 `Pure + Parallel`，其他 Pure runner 也受控并行，Filesystem/SQLite/Avalonia/Windows 必须物理命名且 `Serial`。新增、删除或迁移项目时必须在同一批次更新对应 owner 的项目清单；需要执行 `Get-EdgeTestInventory.ps1`、discovery 或 TRX 的对账必须服从当前批次验证授权。
- 拆仓前 `EdgePluginContractLedgerTests` 的两个 Fact 只验证旧单仓 `src/Modules/IIoT.Edge.Module.Homogenization` 与 canonical ledger/formal 输入，证据唯一保留在冻结 Phase 0 worktree；物理三仓产品 Host 不得复制旧 ledger、改写 generator 路径、生成第二套 ledger或把冻结 worktree接入 required runner。三仓当前组合由 V1–V4 的正式 nupkg、插件 bundle、manifest 与 digest 证明；Host required inventory 必须物理退役这两个前拆仓 Fact，并以真实 discovery/TRX 记录 before/after，不能留缺资产测试或 dummy case。
...
- Windows required CI 保持 25 分钟 hard timeout、无 soft-fail，并运行 Release build、Analyzer/project-graph 与真实正反例 build fixture、inventory、真实 discovery、全部 required runner、TRX/coverage 和 0 failed/0 skipped 对账；不得按 filter 把一个项目拆成多份后用人工加总冒充一次完整执行。物理三仓候选正式关单时必须覆盖 Host 26、SDK 2、Private Plugins 4 个 owner runner，不能把 Host 单仓 26 个 runner 冒充拆仓前或三仓合计 32 个 runner 已执行；在三仓 CI/发布入口获得单独授权并完成改造前，旧单仓 workflow 只能视为待迁移入口。Mutation 使用独立 report job；Windows 远端和实机未运行时必须明确标注，不得用 macOS 本地结果冒充。
- `IIoT.Edge.Module.Analyzers` 必须作为固定 `2.0.0` SDK 包随所有 Host 生产项目构建执行，源码与 `IIoT.Edge.Architecture.AnalyzerTests` 由 SDK 仓独占且保持 `Architecture` + `Pure/Roslyn`。Host build fixture 必须锁定根级 PackageReference metadata、解析后的包根和 Shell project-graph 参数，证明测试/fixture 排除不扩大、生产项目无法通过属性禁用且不会恢复源码 `ProjectReference`。MSBuild `Exec` 向子进程传递带引号的仓根/solution 时必须使用 canonical 绝对路径且目录参数不得带尾随分隔符，避免 Windows 反斜杠吃掉闭合引号；固定主仓路径不得退化为可由 CLI 覆盖的 property。当前启用的 23 个 compiler diagnostics、160 条 SDK AnalyzerTests、7 个 valid / 43 个 invalid Host 隔离 build fixture 和项目图 `WSARCH001` 门禁以 `docs/Edge架构边界契约.md` 为唯一语义来源；未启用的 Rule ID 仍是后续批次，不得因本批已建 Analyzer 而冒充 Edge 全部架构治理完成。

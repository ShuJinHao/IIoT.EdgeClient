...
- 日常跨项目入口是 `../../deploy/Deploy-Changed.ps1`；它要求 Host/SDK/Private Plugins 三仓 clean、已提交的 `main`，只允许 push 现有 HEAD，禁止创建提交或修改任何 tracked 文件。普通部署复用同 SHA 证据，只补受影响 Architecture/Security/DeploymentContract；不运行全量、coverage、mutation、duplication 或三端对齐。Edge“部署”只表示 Mac 本机编译受影响 Windows 宿主/安装器/Velopack 或独立插件，上传并登记 release，再验证下载链；不表示远程安装某台 Windows，也不使用 Harbor。
- 三端从零部署只走 `../../deploy/Deploy-FromZero.ps1`。Cloud 清空后允许按 checkpoint 重试重新签发 Edge Release API key并写回 Keychain；重试必须先吊销同一 invocation 前缀的旧 active 派生 key，但不得自动创建设备、注册 `ClientCode` 或轮换任何设备 bootstrap secret。导回的 release history 行不得删除或覆盖：Host 使用恢复历史之后的新 patch；已有同版本插件只允许从冻结候选重建后对比 hash/size 并验证下载，字节不一致必须停止并要求先修改业务版本。
...
- 只有宿主运行时代码受影响才允许发布 EdgeHost；改动完全位于 Private Plugins 真实模块目录时只允许发布对应 EdgePlugin；SDK Contracts/Module.Sdk/UI.Shared 或 SDK build 输入变化至少影响 Host 并触发唯一兼容门，Analyzer 与纯文档变化不得发布 Host/Plugin。影响无法安全归属必须停止，禁止插件改动顺带发布宿主或未知影响退化全量。`scripts/tests/TestEdgeDeploymentPolicy.ps1` 只由受影响 CI 选择器、普通部署缺口验证或用户显式质量任务调用；普通 Shell build 不得借 MSBuild hook 运行它。任何恢复 `scp/rsync`、绕过 pushed HEAD、混合宿主/插件源码 owner、恢复 Host pack 或删除 Windows 下载语义的改动必须让对应部署契约验证失败。

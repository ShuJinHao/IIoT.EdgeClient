...
- 宿主、SDK 与私有插件三仓拆分由 `docs/Edge宿主SDK私有插件三仓拆分计划.md` 专门裁决。2026-07-22 已形成三个独立 Git 仓：依赖方向唯一为 `Host -> SDK 包`、`Plugin -> SDK 包`，Host 与 Plugin 互不引用；本地 V1–V5 已完成独立 restore/build/pack、零源码 fallback 组合、各仓 owner runner、Host TRX/coverage 对账。Phase 9 唯一发布入口三仓影响映射和 Phase 10 Launcher 组合升级保护已进入产品候选，两个新私有 GitHub 远端已建立；完整兼容门、最终 push、`stable` 发布、Windows 下载与实机结果必须以本批最新复盘和发布交接为准，未实际执行的项目继续明确标为未执行。旧 Phase 0–5 机器账本与 formal 规则只绑定冻结 worktree，不得在产品候选中重生成或冒充新鲜验证。
...
- 工作区日常生产增量发布唯一入口是 `../../deploy/Deploy-Changed.ps1`；`../../deploy/Invoke-WorkspaceDeploy.ps1 -Target EdgeHost|EdgePlugin ...` 只作为其内部执行器和显式恢复入口。`scripts/TestEdgeDeploymentPreflight.ps1` 可以单独执行只读门禁，`LocalPublishAndDeploy.ps1` 和 `PublishEdgePluginRelease.ps1` 只接受统一入口生成的单次内部调度标记，禁止操作者或 AI 直接调用实现脚本绕过 preflight、摘要和取消治理。
...
- 日常跨项目一键入口是 `../../deploy/Deploy-Changed.ps1`；它分别要求 `IIoT.EdgeClient`、`IIoT.Edge.Sdk`、`IIoT.Edge.Plugins.Private` 三仓 clean `main`，自动 fetch/fast-forward/push 各自 HEAD，宿主基线来自 Cloud 已发布 manifest 的 `sourceCommit`，SDK 与 Private Plugins 基线来自各自远端 `deployment-baselines/edge-stable-win-x64` ref。Edge“部署”只表示 Mac 本机编译 Windows 安装器/宿主/Velopack 或独立插件，上传服务器并登记 release，最终验证 Windows 可以从下载中心和更新地址下载；不表示远程安装某台 Windows，也不使用 Harbor。
...
- 只有宿主运行时代码受影响才允许发布 EdgeHost；改动完全位于 Private Plugins 真实模块目录时只允许发布对应 EdgePlugin；SDK Contracts/Module.Sdk/UI.Shared 或 SDK build 输入变化至少影响 Host 并触发唯一兼容门，Analyzer 与纯文档变化不得发布 Host/Plugin。影响无法安全归属必须停止，禁止插件改动顺带发布宿主或未知影响退化全量。`scripts/tests/TestEdgeDeploymentPolicy.ps1` 必须接入 Shell build 和 Edge CI，任何恢复 `scp/rsync`、绕过 pushed HEAD、混合宿主/插件源码 owner、恢复 Host pack 或删除 Windows 下载语义的改动必须让构建失败。

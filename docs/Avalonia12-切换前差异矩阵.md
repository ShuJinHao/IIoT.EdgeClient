...
当前 Avalonia 客户端已经具备本地发布、UI-only 启动、显式 `--start-runtime` 联调入口、只读诊断、I/O 运行时缓冲写入申请、现场证据包采集和 WPF 回退构建验证能力。它还不能直接替换生产 WPF 客户端，生产切换必须先完成 FullGate、现场证据审查、P0/P1 问题回收和切默认入口评审。

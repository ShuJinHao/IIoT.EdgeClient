...
现行更正（2026-07-24）：上段中“隔离 build fixture 锁定 Shell project-graph 参数”“Shell project-graph `Exec` 固定主仓路径”的旧口径不再适用；仓库级 project-graph 必须由默认 CI 显式运行，普通 Shell build 不得隐藏该仓库级测试。只有隔离 graph fixture 允许受限 MSBuild target，fixture root/solution 的 canonical 绝对路径、去尾分隔符与显式 fixture role 约束继续有效；本更正不弱化 Analyzer、project-graph 或 WSARCH006 的任何诊断规则。

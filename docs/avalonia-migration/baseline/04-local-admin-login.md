...
Avalonia 迁移的任何 Phase 中，`PasswordHash` 字段值绝对不允许清空、覆盖、删除或重新生成。禁止用默认密码、示例密码、`LauncherPasswordHasher.Compute(...)` 或任何脚本改写存量账号哈希；即便密码语义相同，也必须保留原文件中的原值。

Phase 0 盘点确认本地管理员账号文件属于存量业务配置。后续 Phase 必须把账号文件视为只读业务配置，只允许读取验证，不允许改写。
